...
export default function AdminPage() {
  ...
  async function refresh() {
    const { data: f, error: fe } = await supabase.from("formations").select("*").order("created_at", { ascending: false });
    if (fe) alert(fe.message);
    setFormations((f ?? []) as any);

    const { data: m, error: me } = await supabase.from("membres").select("id, nom, email, role").order("created_at", { ascending: false });
